...
def train_epoch_audio(epoch, model, dataloader, optimizer, scheduler,device, writer=None):
    ...
    for step, (spec, label) in (pbar := tqdm(enumerate(dataloader), desc='Epoch: {}: '.format(epoch))):
        #pdb.set_trace()
        spec = spec.to(device)
